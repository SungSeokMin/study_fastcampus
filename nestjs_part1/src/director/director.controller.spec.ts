...
describe('DirectorController', () => {
  ...
  describe('create', () => {
    it('should call create method from DirectorService with correct DTO', () => {
      const createDirectorDto = { name: 'sung' };
      const result = { id: 1, name: 'sung' };

      jest.spyOn(mockDirectorService, 'create').mockResolvedValue(result);

      expect(controller.create(createDirectorDto as CreateDirectorDto)).resolves.toEqual(result);
  ...
  describe('update', () => {
    it('should call update method from DirectorService with correct ID and DTO', async () => {
      const updateDirectorDto = { name: 'sung' };
      const result = {
        id: 1,
        name: 'sung',
      };
