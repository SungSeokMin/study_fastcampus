...
describe('MovieService - Integration Test', () => {
  ...
  beforeEach(async () => {
    ...
    users = [1, 2].map((x) =>
      userRepository.create({ id: x, email: `${x}@test.com`, password: '123123' }),
    );
    ...
    movies = Array.from({ length: 15 }, (_, x) => x + 1).map((x) =>
      movieRepository.create({
        ...
        likeCount: 0,
        dislikeCount: 0,
        detail: movieDetailRepository.create({ detail: `Movie Detail ${x}` }),
        movieFilePath: 'movies/movie1.mp4',
        director: directors[0],
        createdAt: new Date(`2025-09-${x}`),
      }),
    );
  ...
  describe('update', () => {
    ...
    it('should throw error if movie does not exist', async () => {
      ...
      await expect(service.update(999999, updateMovieDto)).rejects.toThrow(NotFoundException);
    });
  });
